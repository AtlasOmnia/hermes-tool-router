...
def _minimal_prompt_agent(**overrides):
    values = {
...
class TestBuildContextFilesPrompt:
    ...
    def test_load_compact_soul_prefers_dedicated_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("HERMES_HOME", str(tmp_path / "hermes_home"))
        hermes_home = tmp_path / "hermes_home"
        hermes_home.mkdir()
        (hermes_home / "SOUL.md").write_text("Full identity.", encoding="utf-8")
        (hermes_home / "SOUL-compact.md").write_text("Compact identity.", encoding="utf-8")

        assert load_compact_soul_md() == "Compact identity."

    def test_load_compact_soul_uses_marked_section(self, tmp_path, monkeypatch):
        monkeypatch.setenv("HERMES_HOME", str(tmp_path / "hermes_home"))
        hermes_home = tmp_path / "hermes_home"
        hermes_home.mkdir()
        (hermes_home / "SOUL.md").write_text(
    ...
    def test_load_compact_soul_start_marker_without_end_uses_tail(self, tmp_path, monkeypatch):
        monkeypatch.setenv("HERMES_HOME", str(tmp_path / "hermes_home"))
        hermes_home = tmp_path / "hermes_home"
        hermes_home.mkdir()
        (hermes_home / "SOUL.md").write_text(
            "Verbose opening.\n"
            "<!-- compact-start -->\n"
            "Compact identity to EOF.",
            encoding="utf-8",
        )

        assert load_compact_soul_md() == "Compact identity to EOF."

    def test_load_compact_soul_truncates_unmarked_soul(self, tmp_path, monkeypatch):
        monkeypatch.setenv("HERMES_HOME", str(tmp_path / "hermes_home"))
        hermes_home = tmp_path / "hermes_home"
        hermes_home.mkdir()
        (hermes_home / "SOUL.md").write_text("abcdef", encoding="utf-8")

        result = load_compact_soul_md(max_chars=3)
        assert result.startswith("abc")
        assert "def" not in result

    def test_compact_system_prompt_uses_compact_soul(self, tmp_path, monkeypatch):
        ...
        (hermes_home / "SOUL.md").write_text(
            ...
            "Compact identity.\n"
            "<!-- compact-end -->\n"
            "Private details that compact turns should omit.",
            encoding="utf-8",
        )

        prompt = build_system_prompt(_minimal_prompt_agent(_memory_tier="compact"))

        assert "Compact identity." in prompt
        assert "Verbose identity." not in prompt
        assert "Private details" not in prompt
        assert "F.R.I." + "D.A.Y." not in prompt
        assert "Jona" + "than" not in prompt

    def test_system_prompt_defaults_without_soul(self, tmp_path, monkeypatch):
        from agent.system_prompt import build_system_prompt
