...
class TestAgentConfigSignature:
    ...
    def test_router_message_hash_busts_cache(self):
        """Router-enabled sessions need prompt-specific schemas."""
        from gateway.run import GatewayRunner

        runtime = {"api_key": "k", "base_url": "u", "provider": "p"}
        msg1 = hashlib.sha256(b"what files changed?").hexdigest()
        msg2 = hashlib.sha256(b"search the web for current docs").hexdigest()
        sig1 = GatewayRunner._agent_config_signature(
            "m",
            runtime,
            ["telegram"],
            "",
            cache_keys={"router_user_message_sha256": msg1},
        )
        sig2 = GatewayRunner._agent_config_signature(
            "m",
            runtime,
            ["telegram"],
            "",
            cache_keys={"router_user_message_sha256": msg2},
        )
