...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    memory_tier = getattr(agent, "_memory_tier", "full")
    if memory_tier not in {"none", "compact", "full"}:
        memory_tier = "full"
    logger.info("memory_tier: %s", memory_tier)

    # ── Stable tier ────────────────────────────────────────────────
    stable_parts: List[str] = []

    # Try SOUL.md as identity/guidance unless explicitly skipped.
    # Some execution modes (cron) still want HERMES_HOME persona while keeping
    # cwd project instructions disabled.
    _soul_loaded = False
    if agent.load_soul_identity or not agent.skip_context_files:
        if memory_tier == "compact":
            _soul_content = _r.load_compact_soul_md()
        else:
    ...
    if not agent.skip_context_files:
        # Routed trivial openers ('.', 'hi', etc.) are handshake turns, not
        # project work. Loading AGENTS.md/CLAUDE.md for them dominates real
        # Codex input tokens and defeats the router/memory narrow path.
        # Keep context files for clear tasks and history-heavy turns.
        _suppress_context_for_handshake = (
            getattr(agent, "_memory_tier", None) == "none"
            and bool(getattr(agent, "_router_predicted_toolsets", None))
        )
        if not _suppress_context_for_handshake:
    ...
    # Compact-tier live-data hint: without full memory context the agent can
    # mistake "search" for local file search. Reinforce tool choice for the
    # most common misdirection pattern seen in quality regression (2026-05-26).
    if memory_tier == "compact":
        context_parts.append(
            "For queries about current prices, recent news, or any live data: "
            "use web_search — not search_files (local filesystem) or memory lookup."
        )

    # ── Volatile tier (changes per session/turn — never cached) ───
    volatile_parts: List[str] = []

    if agent._memory_store and memory_tier != "none":
        if agent._memory_enabled and memory_tier == "full":
            mem_block = agent._memory_store.format_for_system_prompt("memory")
            if mem_block:
                volatile_parts.append(mem_block)
        # USER.md is included for compact/full tiers; tier none omits it.
