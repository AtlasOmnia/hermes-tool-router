...
COMPACT_SOUL_MAX_CHARS = 800
COMPACT_SOUL_START_MARKER = "<!-- compact-start -->"
COMPACT_SOUL_END_MARKER = "<!-- compact-end -->"
...
OPENAI_MODEL_EXECUTION_GUIDANCE = (
    ...
    "- Current time, date, timezone → use terminal. For formatted answers, use Python datetime (`python3 -c ...`); avoid shell `date` formatting flags such as `%-d` or `date -d` unless verified.\n"
...
def _compact_soul_limit(max_chars: Optional[int] = None) -> int:
    if max_chars is not None:
        return max(1, int(max_chars))
    raw = os.getenv("HERMES_COMPACT_SOUL_MAX_CHARS", "").strip()
    if raw:
        try:
            return max(1, int(raw))
        except ValueError:
            logger.debug("Ignoring invalid HERMES_COMPACT_SOUL_MAX_CHARS=%r", raw)
    return COMPACT_SOUL_MAX_CHARS


def _truncate_compact_soul(content: str, max_chars: int) -> str:
    if len(content) <= max_chars:
        return content
    return content[:max_chars].rstrip() + "\n\n[...compact SOUL.md truncated...]"


def _extract_compact_soul(content: str, max_chars: int) -> str:
    """Extract user-curated compact identity text from SOUL.md when present."""
    content = _strip_yaml_frontmatter(content).strip()
    start = content.find(COMPACT_SOUL_START_MARKER)
    end = content.find(COMPACT_SOUL_END_MARKER)
    if start != -1:
        body_start = start + len(COMPACT_SOUL_START_MARKER)
        if end != -1 and end > body_start:
            return content[body_start:end].strip()
        return _truncate_compact_soul(content[body_start:].strip(), max_chars)
    if end != -1:
        return content[:end].strip()
    return _truncate_compact_soul(content, max_chars)


def load_compact_soul_md(max_chars: Optional[int] = None) -> Optional[str]:
    """Load a compact agent identity from HERMES_HOME.

    Resolution order:
      1. ``SOUL-compact.md`` when present.
      2. ``SOUL.md`` content between compact markers.
      3. ``SOUL.md`` content before ``<!-- compact-end -->``.
      4. A truncated prefix of ``SOUL.md``.

    Returns None when no usable compact identity exists. Callers should fall
    back to ``DEFAULT_AGENT_IDENTITY``.
    """
    try:
        from hermes_cli.config import ensure_hermes_home
        ensure_hermes_home()
    except Exception as e:
        logger.debug("Could not ensure HERMES_HOME before loading compact SOUL.md: %s", e)

    hermes_home = get_hermes_home()
    limit = _compact_soul_limit(max_chars)

    compact_path = hermes_home / "SOUL-compact.md"
    try:
        if compact_path.exists():
            content = compact_path.read_text(encoding="utf-8").strip()
            if content:
                content = _strip_yaml_frontmatter(content).strip()
                content = _truncate_compact_soul(content, limit)
                return _scan_context_content(content, "SOUL-compact.md")
    except Exception as e:
        logger.debug("Could not read compact SOUL.md from %s: %s", compact_path, e)
    ...
    try:
        ...
        if not content:
            return None
        content = _extract_compact_soul(content, limit)
        if not content:
            return None
        return _scan_context_content(content, "SOUL.md")
    except Exception as e:
        logger.debug("Could not read compact SOUL.md from %s: %s", soul_path, e)
        return None
