...
TERMINAL_TOOL_DESCRIPTION = """Execute shell commands in the configured terminal backend. Filesystem usually persists between calls.
...
For current date, time, or timezone checks, use plain `date`/`date -u` only when raw output is enough. For formatted answers, use Python datetime, for example `python3 -c "from datetime import datetime; print(datetime.now().astimezone().strftime('%A, %B %d, %Y'))"`. Avoid shell `date` formatting flags such as `%-d` or `date -d` unless you have verified they work in the active backend.
