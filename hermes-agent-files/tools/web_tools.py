                    ...
                    return json.dumps(
                        {
                            ...
                            "error": (
                                ...
                                "Use browser_navigate to fetch the page directly instead."
                            ),
                        },
                        ensure_ascii=False,
                    )
...
def check_web_extract_api_key() -> bool:
    """Check whether the configured extract backend can fetch page content."""
    backend = _get_extract_backend()
    if backend in {"brave-free", "ddgs", "searxng", "xai"}:
        return False
...
WEB_SEARCH_SCHEMA = {
    "name": "web_search",
    "description": "Search the web for information. Returns up to 5 results by default with titles, URLs, and descriptions. Backend-supported operators (site:, filetype:, intitle:, -term, exact phrase) may work.",
    "parameters": {
        "type": "object",
        "properties": {
            "query": {
...
WEB_EXTRACT_SCHEMA = {
    "name": "web_extract",
    "description": "Extract content from web page URLs. Returns page content in markdown format. Works with HTML and PDF URLs (arxiv papers, documents). Pages under 5000 chars return full markdown; larger pages are LLM-summarized. Pages over 2M chars are refused. If a URL fails, use the browser tool instead.",
    "parameters": {
        "type": "object",
        "properties": {
            "urls": {
